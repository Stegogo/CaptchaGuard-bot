...
async def process_callback_button1(callback_query: CallbackQuery):
    if commhandlers.data.id == callback_query.from_user.id:
        ...
        await bot.send_message(callback_query.message.chat.id, _('Правильно!'))
        ...
        await bot.answer_callback_query(
            callback_query.id,
            text=(_('Эта капча не для тебя 😉')), show_alert=True)
...
async def process_callback_button1(callback_query: CallbackQuery):
    if commhandlers.data.id == callback_query.from_user.id:
        ...
        await bot.delete_message(chat_id=callback_query.message.chat.id, message_id=pic_id - 1)
        await bot.delete_message(chat_id=callback_query.message.chat.id, message_id=pic_id)
        await bot.send_message(callback_query.message.chat.id, (_(f'Пользователь провалил капчу! Ответ выбран неправильно. Пользователь исключён из чата.')))
